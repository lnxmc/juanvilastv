...
export default function HeroSection({ videoUrl }: { videoUrl?: string }) {
  ...
  return (
    <section id="hero">
      ...
      {/* Grid principal */}
      <div className={`hero-grid${videoId ? '' : ' no-video'}`}>

        {/* Texto */}
        <div>
          <div style={{ fontFamily: "'Space Mono', monospace", fontSize: 'clamp(0.6rem, 1.5vw, 0.72rem)', letterSpacing: '0.2em', textTransform: 'uppercase', color: 'var(--gold)', marginBottom: '1.2rem' }}>
            ↳ Temporada 2026 · Jaén · 1.000 olivos Picual
          </div>

          <h1 style={{ fontFamily: "'Playfair Display', serif", fontSize: 'clamp(2.8rem, 8vw, 6rem)', fontWeight: 900, lineHeight: 1.0, color: 'var(--cream)' }}>
            Juan Vilas
          ...
          <p style={{ marginTop: '1.5rem', fontSize: 'clamp(1rem, 2vw, 1.15rem)', color: 'var(--cream-dim)', maxWidth: 500, fontStyle: 'italic', lineHeight: 1.8 }}>
      ...
      {/* Stats — solo sin video y en desktop */}
      {!videoId && (
        <div className="hero-stats-bottom" style={{ position: 'absolute', bottom: '4rem', right: '3rem', display: 'flex', gap: '3rem' }}>
      ...
      {/* Scroll indicator */}
      <div style={{ position: 'absolute', bottom: '1.5rem', left: '3rem', display: 'flex', alignItems: 'center', gap: '1rem' }}>
        <div style={{ width: 40, height: 1, background: 'var(--gold-muted)' }} />
        <span style={{ fontFamily: "'Space Mono', monospace", fontSize: '0.6rem', letterSpacing: '0.2em', textTransform: 'uppercase', color: 'var(--cream-dim)' }}>Desplázate</span>
      </div>
    </section>
  )
}
